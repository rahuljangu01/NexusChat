// client/src/pages/Dashboard.jsx (FINAL & FULLY RESPONSIVE FIX)
...
const Dashboard = () => {
  ...
  const isContentPageOpen = location.pathname !== '/dashboard';
  ...
  return (
    <div className="h-screen w-screen flex text-gray-200 bg-[#0d1117] overflow-hidden">
      ...
      <div className={`
          ...
          ${isContentPageOpen ? 'hidden md:flex' : 'flex'}
      `}>
          <header className="p-4 h-20 flex-shrink-0 flex items-center justify-between border-b border-slate-800 bg-[#1e293b]/30">
              ...
              <Dialog>
                  <DialogTrigger asChild><Button variant="ghost" className="h-10 w-10 rounded-full bg-indigo-600/20 text-indigo-400 hover:bg-indigo-600/40 hover:text-indigo-300"><UserPlus className="h-5 w-5"/></Button></DialogTrigger>
                  <FindPeersDialog user={user} connections={connections} onAction={{handleAction, pendingRequests}} />
              </Dialog>
          </header>
          
          <div className="p-4 border-b border-slate-800">
              <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500"/><Input placeholder="Search chats..." value={chatSearch} onChange={(e) => setChatSearch(e.target.value)} className="pl-12 h-12 text-base bg-slate-900 border-slate-700 focus:ring-indigo-500 rounded-full" /></div>
          </div>
            ...
            <div className="p-2">
              ...
              <AnimatePresence>
                  {sortedAndFilteredConnections.map(conn => {
                    ...
                    return (
                      <motion.div 
                        layout key={conn._id} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} 
                        onClick={() => navigate(`/dashboard/chat/${otherUser._id}`)} 
                        className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors duration-200 mx-2 ${isActiveChat ? 'bg-slate-700/50' : 'hover:bg-slate-800/50'}`}
                      >
                        ...
                        <div className="flex-1 ml-3 overflow-hidden">
                            <div className="flex justify-between items-center"><h3 className={`font-semibold text-sm truncate ${unreadCount > 0 || isActiveChat ? 'text-white' : 'text-gray-300'}`}>{otherUser.name}</h3>{lastMessage && <p className="text-xs text-slate-400 flex-shrink-0">{format(new Date(lastMessage.createdAt), 'p')}</p>}</div>
                            <div className="flex justify-between items-center mt-1">
                                <p className={`text-xs ${unreadCount > 0 ? 'text-indigo-300 font-semibold' : 'text-gray-400'} truncate`}>{lastMessage?.content || 'No messages yet...'}</p>
                                {unreadCount > 0 && (<span className="bg-indigo-600 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">{unreadCount}</span>)}
                            </div>
                        </div>
                      </motion.div>
                    );
      ...
      {/* Added pb-16 (padding-bottom: 4rem) for mobile and md:pb-0 to remove it on desktop */}
      <main className={`
          flex-1 flex flex-col relative bg-[#0d1117]
          pb-16 md:pb-0
          ${isContentPageOpen ? 'flex' : 'hidden md:flex'}
      `}>
          <Outlet />
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#161b22] border-t border-slate-800 flex justify-around items-center h-16 z-30">
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <div className={`flex flex-col items-center justify-center gap-1 p-1 w-full h-full cursor-pointer ${location.pathname.startsWith('/dashboard/profile') ? 'text-indigo-400' : 'text-slate-400'}`}>
                    <MoreVertical className="h-6 w-6" />
                    <span className="text-xs">More</span>
                </div>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="mb-2 w-48 bg-slate-800 border-slate-700 text-white" side="top" align="end">
                <DropdownMenuItem onClick={() => navigate('/dashboard/profile')}><User className="mr-2 h-4 w-4" /><span>Profile</span></DropdownMenuItem>
                <DropdownMenuItem onClick={() => navigate('/dashboard/about-developer')}><Code className="mr-2 h-4 w-4" /><span>About Dev</span></DropdownMenuItem>
                <DropdownMenuSeparator className="bg-slate-700"/>
                <DropdownMenuItem className="text-red-400" onClick={() => setIsLogoutConfirmOpen(true)}><LogOut className="mr-2 h-4 w-4" /><span>Logout</span></DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
      </nav>
    </div>
