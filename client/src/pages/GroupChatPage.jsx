// client/src/pages/GroupChatPage.jsx (FINAL - FULLY COMPACT UI)
...
import api, { uploadProfilePhoto, logCall } from "../utils/api";
...
const GroupChatPage = () => {
    ...
    const startGroupCall = useCallback((type) => {
        ...
        navigator.mediaDevices.getUserMedia(constraints).then(stream => {
            ...
            socketService.emit('start-group-call', { 
                groupId, 
                from: { id: currentUserId, name: currentUserName, profilePhotoUrl: currentUserPhoto },
                callType: type
            });
        }).catch(err => {
    ...
    const answerGroupCall = useCallback(() => {
        ...
        navigator.mediaDevices.getUserMedia(constraints).then(stream => {
            ...
            socketService.emit('join-group-call', {
                groupId,
                from: { id: currentUserId, name: currentUserName, profilePhotoUrl: currentUserPhoto }
            });
        });
    ...
    useEffect(() => {
        ...
        const handleIncomingCall = (data) => {
            if (data.groupId === groupId && groupCallState === 'idle') {
        ...
        const handleNewUserJoining = ({ from }) => {
            ...
            peer.on('signal', signal => {
                socketService.emit('send-signal-group', { signal, to: from.id, from: { id: currentUserId, name: currentUserName, profilePhotoUrl: currentUserPhoto } });
            });
        ...
        const handleReceivingSignal = ({ signal, from }) => {
            ...
            peer.on('signal', returnSignal => {
                 socketService.emit('return-signal-group', { signal: returnSignal, to: from.id });
            });
            peer.signal(signal);
            peersRef.current[from.id] = { peer, peerID: from.id, name: from.name, profilePhotoUrl: from.profilePhotoUrl };
            setPeers({...peersRef.current});
        ...
        const handleReceivingReturnSignal = ({signal, from}) => {
             const item = peersRef.current[from.id];
            if (item) {
    ...
    const handleFileChange = async (event) => {
        ...
        try {
            ...
            await api.post(`/groups/${groupId}/send`, {
            ...
            <AnimatePresence>
                {groupCallState !== 'idle' && (
                    <GroupCallingUI 
                        ...
                        callType={callType}
                        leaveCall={() => leaveGroupCall(groupCallState === 'incoming')}
                        answerCall={answerGroupCall}
                        isMuted={isMuted}
                        toggleMute={() => {
                            if(stream) {
            ...
            <main className={`flex-1 flex flex-col relative ${groupCallState !== 'idle' ? 'blur-sm pointer-events-none' : ''}`}>
                                  ...
                                  <div key={item._id || index}>
                                    ...
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center my-3">
                                      <div className="flex items-center gap-2 text-sm text-slate-400 bg-slate-800/60 px-3 py-1.5 rounded-lg">
                                        ...
                                        <span>Call started by {callerName}</span>
                                        <span>•</span>
                                        <span>{callTime}</span>
                                      </div>
                                    </motion.div>
                ...
                <footer className="p-2 border-t border-slate-800 bg-slate-900/70 backdrop-blur-lg z-20 relative">
                    ...
                    <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                        ...
                        <Button type="button" variant="ghost" size="icon" className="h-10 w-10 text-gray-400 hover:text-white flex-shrink-0" onClick={() => setShowEmojiPicker(!showEmojiPicker)}>
                        ...
                        <Input 
                            ...
                            className="flex-1 h-10 bg-slate-800 border-slate-700 rounded-full px-4" 
                            autoComplete="off"
                            onFocus={() => setShowEmojiPicker(false)}
                        />
                        <Button type="submit" size="icon" className="h-10 w-10 bg-indigo-600 hover:bg-indigo-500 rounded-full flex-shrink-0" disabled={!newMessage.trim() || isUploading}>
                            {isUploading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> : <Send className="h-5 w-5" />}
                        </Button>
                    </form>
                </footer>
            </main>
                ...
                {isInfoPanelOpen && (
                    <motion.aside initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="w-full md:w-[350px] bg-[#161b22] flex flex-col border-l border-slate-800 z-30 absolute top-0 right-0 h-full">
                        ...
                        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
                            ...
                            <div className="flex justify-between items-center mb-2"><h4 className="text-sm uppercase font-semibold text-slate-500">{group.members.length} Members</h4> {isAdmin && <Button variant="ghost" size="icon" className="h-8 w-8 text-indigo-400"><UserPlus className="h-5 w-5"/></Button>}</div>
                            <TooltipProvider>
                                        ...
                                        <div 
                                          ...
                                          onClick={() => {
                                            if (member.user._id !== currentUserId) {
                                                navigate(`/dashboard/chat/${member.user._id}`)
                                            }
                                          }}
                                        >
